...
async def chat_with_claude(user_input, model_choice, image_path=None, current_iteration=None, max_iterations=None):
...
async def main(user_input, model_choice):
    ...
    while True:
        ...
        if user_input.lower() == 'image':
            ...
            response, _ = await chat_with_claude(user_input, model_choice)
            return response

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Claude Chatbot")
    parser.add_argument('model_choice', type=str, help='Model choice for Claude (e.g., claude-3-5-sonnet)')
    parser.add_argument('user_input', type=str, help='User input for Claude')

    args = parser.parse_args()

    asyncio.run(main(args.user_input, args.model_choice))
